...
{
    public class VectorLayerVisualizer
    {
        public Dictionary<int, VectorFeatureProcessor> GetModStacks => _stackList;
        public bool Active { get; set; }
        public string VectorLayerName { get; private set; }
        
        protected VectorLayerVisualizerSettings _settings;
        protected UnityContext _unityContext;
        protected Dictionary<int, VectorFeatureProcessor> _stackList;
        ...
        public virtual void UpdateForView(CanonicalTileId canonicalTileId, IMapInformation information)
        {
            if (_results.TryGetValue(canonicalTileId, out var visuals))
            {
                foreach (var entity in visuals)
                {
                    ...
                    // if (stack.Settings.LayerType == FeatureGeometryType.Polygon || stack.Settings.LayerType == FeatureGeometryType.Line)
                    // {
